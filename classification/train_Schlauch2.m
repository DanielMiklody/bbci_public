function C = train_Schlauch2(xTr, yTr, varargin)
...
if nargin<4||~isnumeric(varargin{1})||~isnumeric(varargin{2})
    error('Schlauch needs yo Eigenvalues and freedom')
end
...
if isempty(opt_shrinkage)||any(opt_shrinkage.Gamma>0    )
...
C.D=varargin{1};
C.k=varargin{2};
C.w=C_invcov*C.k/2*(1./(1-C.D)-1./C.D);
%C.w=C.D./diag(C_cov).*C.k/2.*(1./(1-C.D)-1./C.D);
C.b=(C.k/2-1)*sum(log(1./C.D-1));
