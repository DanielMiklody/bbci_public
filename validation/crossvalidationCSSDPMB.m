function [loss, lossSem, cfy_out]= crossvalidationCSSDPMB(cnt,mrk,freqs, ival,varargin)
...
misc_checkType(mrk, 'STRUCT( y)');
misc_checkType(cnt, 'STRUCT( x)');
% misc_checkType(cnt.x, 'DOUBLE[2- 1]|DOUBLE[1- 2-]|DOUBLE[- - -]', 'fv.x');
...
[divTr, divTe]= sampleFcn(mrk.y, samplePar{:});
[lossFcn, lossPar]= misc_getFuncParam(opt.LossFcn);
nFreq=size(freqs{1},2);
[filt_b,filt_a]=butters(1,freqs{1}/cnt.fs*2);
% [filt_b,filt_a]= butter(opt.filterOrder, freqs(1,:)/cnt.fs*2);
% cnt_filt=proc_filt(cnt,filt_b, filt_a);
cnt_filt=proc_filterbank(cnt,filt_b, filt_a);
epo=proc_segmentation(cnt_filt,mrk,ival);
...
nOutDim= size(mrk.y,1);
if nOutDim==2,
  nOutDim= 1;
end
cfy_out= NaN*zeros(nOutDim, length(divTe), size(mrk.y,2));
for rr= 1:length(divTr),
  ...
  for ff= 1:nFolds,      
    ...
    W=cell(1,nFreq);
    for ii=1:nFreq
        freqs2=[freqs{1}(ii,:);freqs{2}(ii,:);freqs{3}(ii,:)];
        [~,W{ii}]=proc_cssdp(cnt,mrk_selectEvents(mrk,idxTr),freqs2,'ival',ival, ...
        'filterOrder',opt.filterOrder,'SelectFcn',opt.SelectFcn);
    end
    fv=proc_multiBandLinearDerivation(epo,W);
