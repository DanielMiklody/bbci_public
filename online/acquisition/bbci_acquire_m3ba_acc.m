function varargout= bbci_acquire_m3ba(varargin)
...
if isequal(varargin{1}, 'init'),
    ...
    for q= 1:3,
        ...
        m3ba_getdata( state.sp, [], 1 );
    end
    %     state.packetNo=[];
    state.buffer= [];
    state.lastx= nan(1, state.nChans);
...
elseif length(varargin)~=1,
    ...
    %  pause(0.01);
    %end
    [ packet, state.buffer ] = m3ba_getdata( sp, state.buffer, expackets )
    ...
    switch packet.type,
        case 'NIR',
            ...
            %     cntx= reshape(cntx, [state.nChans np])';
        case 'EEG',
            cntx(1,NoNIRSCh+1:NoNIRSCh+state.NoEEGCh)= state.scale * packet.data;
        case 'MSG',
            mrkTime=0;
            mrkDesc=num2str(packet.message);
