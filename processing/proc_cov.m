function [dat, varargout]= proc_cov(dat, varargin)
...
nSample= size(dat.x, 1);
nChans= size(dat.x, 2);
nEpo=size(dat.x,3);
Covs= zeros(nChans, nChans, nEpo);
for k= 1:nEpo,
  Covs(:,:,k)= covFcn(dat.x(:,:,k), covPar{:});
end
...
% dat= proc_linearDerivation(dat, W, 'prependix','scssp');
