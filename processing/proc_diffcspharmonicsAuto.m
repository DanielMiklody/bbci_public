function [dat, varargout]= proc_diffcspharmonicsAuto(dat,datband2, varargin)
...
if isempty(opt.weight)
    opt.weight=ones(1,2*size(dat.y,2)) ;
end
...
if isnumeric(opt.covPolicy),
  ...
  %R= procutil_covClasswise(proc_appendEpochs( dat, datband2), opt);
  R= procutil_covClasswise( dat, opt);
  R2= procutil_covClasswise( datband2, opt);
  R=R/trace(sum(R,3));
  R2=R2/trace(sum(R2,3));
end

%% do actual CSP calculation as generalized eigenvalues
%[W,D]= eig(R(:,:,1)-R(:,:,2),R(:,:,1)+R(:,:,2));
%[W2,D2]= eig(R2(:,:,1)-R2(:,:,2),R2(:,:,1)+R2(:,:,2));
[W,D]= eig(R(:,:,1)+R2(:,:,1)-R(:,:,2)-R2(:,:,2),sum(R,3)+sum(R2,3));
...
datband2= proc_linearDerivation(datband2, Wp, 'prependix','csp');
dat=proc_appendChannels(dat,datband2);
