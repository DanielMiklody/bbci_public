function [fv, varargout]= proc_sCSPAutoFreq_orig(dat, freqs, varargin)
...
%band= select_bandbroad(dat_lap, 'band',freqs(iFreq,:));
if band(1)==band(2)
    band=freqs(iFreq,:);
end
bands=[bands;band];
...
for ifreq=1:size(freqs,1)
    ...
    [fv_i, csp_w_i,csp_a_i,csp_score_i]=proc_sCSPAuto_orig(epo,opt_pickProps(opt, proc_sCSPAuto()));
