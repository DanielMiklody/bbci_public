function bands= select_bandsnarrowharmonics(dat, varargin)
...
idx1= find(score.t>=opt.bandTopscore(1) & ...
          score.t<=opt.bandTopscore(2));
idx2= 2*idx1;
chanscore1= sqrt(sum(score.x(idx1,:).^2, 1));
chanscore2= sqrt(sum(score.x(idx2,:).^2, 1));
chanscore=sqrt(sum(score.x(unique([idx1, idx2]),:).^2, 1));
aaa = 1;
...
tempscore= mean(abs(score.x(idx1,chansel)+score.x(idx2,chansel)),2);
[dmy, topidx]= max(tempscore);
topfreq= idx1(topidx);

%% correct sign and redo
idx1_b= find(score.t>=opt.band1(1) & ...
          score.t<=opt.band1(2));
xx= score.x(idx1_b,chansel);
