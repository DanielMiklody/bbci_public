function band= select_bandnarrow_epo(epo, varargin)
...
done_laplace= regexpi(epo.clab,'^\w+ lap\w*');
...
misc_checkType(epo, 'STRUCT(x y clab fs)');
...
if opt.doLaplace,
    epo= proc_laplacian(epo,'requireCompleteNeighborhood', opt.laplaceRequireNeighborhood);
end
...
spec= proc_selectChannels(epo,  cat(2, opt.areas{:}));
