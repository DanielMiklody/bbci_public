function [dat2, varargout]= proc_scssp_orig_onlineNoiseupdate(dat, W,score,C, varargin)
...
for ii=1:nEpo
    X_n=epo_noise.x(:,:,ii);
    X_c=dat.x(:,:,ii);
    C_n= (1-opt.lambda)*eye(nChans)+opt.lambda*W'*(covFcn(X_c, covPar{:})+opt.alpha*covFcn(X_n, covPar{:}))*W;
